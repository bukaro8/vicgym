...
export default async function SummaryPage({ params }: PageProps<"/workouts/[sessionId]/summary">) {
  ...
  return <AppShell><main className="mx-auto w-full max-w-2xl flex-1 px-4 py-10 text-center sm:px-8">
    ...
    <div className="mt-7 grid grid-cols-3 gap-3 text-left"><div className="rounded-2xl border bg-card p-4"><CheckCircle2 className="size-5 text-primary"/><p className="mt-3 text-2xl font-semibold">{completedExercises}</p><p className="text-xs text-muted-foreground">Exercises complete</p></div><div className="rounded-2xl border bg-card p-4"><ListChecks className="size-5 text-primary"/><p className="mt-3 text-2xl font-semibold">{completedSets}</p><p className="text-xs text-muted-foreground">Completed sets</p></div><div className="rounded-2xl border bg-card p-4"><Clock3 className="size-5 text-primary"/><p className="mt-3 text-2xl font-semibold">{duration}m</p><p className="text-xs text-muted-foreground">Duration</p></div></div>
