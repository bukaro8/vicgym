...
export default async function FinishPage({ params }: PageProps<"/workouts/[sessionId]/finish">) {
  ...
  return <AppShell><main className="mx-auto w-full max-w-2xl flex-1 px-4 py-7 sm:px-8 sm:py-10">
    ...
    <section className="mt-6 rounded-3xl border bg-card p-5"><h2 className="text-lg font-semibold">Exercise status</h2><ul className="mt-4 space-y-3">{session.exerciseSessions.map((item) => { const count = item.setLogs.filter((set) => set.completedAt).length; return <li key={item.id} className="flex items-center justify-between gap-3 border-t pt-3 first:border-0 first:pt-0"><span className="font-medium">{item.exerciseNameSnapshot}{item.isAdHoc && <span className="ml-2 rounded-full bg-accent px-2 py-1 text-xs text-primary">Extra</span>}</span><span className="text-sm text-muted-foreground">{count}/{item.plannedSets} planned sets</span></li>; })}</ul></section>
