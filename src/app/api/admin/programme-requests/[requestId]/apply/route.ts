...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  try { assertSameOriginJson(request); await requireAdminUser(); const { requestId } = await context.params; const { json } = bodySchema.parse(await request.json()); const result = await applyRequestProgramme(getPrisma(), requestId, json); revalidatePath("/admin/requests"); revalidatePath(`/admin/requests/${requestId}`); return NextResponse.json({ applied: true, program: result.program, versionNumber: result.versionNumber }); }
