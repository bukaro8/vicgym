...
const mutationSchema = z.object({ id: z.string().uuid(), sequence: z.number().int().positive(), type: z.enum(["ADD_SET", "UPSERT_SET", "UPSERT_TIMER", "FINISH_WORKOUT"]), sessionId: z.string().uuid(), targetId: z.string().uuid(), payload: z.record(z.string(), z.unknown()), createdAt: z.string().datetime(), attempts: z.number().int().nonnegative(), lastError: z.string().nullable() });
