...
const mutationSchema = z.object({ id: z.string().uuid(), sequence: z.number().int().positive(), type: z.enum(["ADD_EXERCISE", "ADD_SET", "UPSERT_SET", "UPSERT_TIMER", "UPDATE_CARDIO", "FINISH_WORKOUT"]), sessionId: z.string().uuid(), targetId: z.string().uuid(), payload: z.record(z.string(), z.unknown()), createdAt: z.string().datetime(), attempts: z.number().int().nonnegative(), lastError: z.string().nullable() });
const batchSchema = z.object({ schemaVersion: z.literal(1), mutations: z.array(mutationSchema).min(1).max(100) });

export async function POST(request: Request) {
  try {
    assertSameOriginJson(request); const user = await requireApiUser(); const input = batchSchema.parse(await request.json()); const results = await replayOfflineMutations(getPrisma(), user.id, input.mutations); const failed = results.some((result) => result.status === "failed");
