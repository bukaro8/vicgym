...
export async function POST(request: Request, { params }: { params: Promise<{ sessionId: string; exerciseSessionId: string }> }) {
  try {
    ...
    const set = await getPrisma().$transaction(async (tx) => {
      const exercise = await tx.exerciseSession.findFirst({ where: { id: exerciseSessionId, workoutSessionId: sessionId, workoutSession: { userId: user.id, status: "IN_PROGRESS" } }, include: { setLogs: { orderBy: { setNumber: "desc" }, take: 1 } } });
