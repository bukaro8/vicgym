...
export async function POST(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    await getPrisma().$transaction(async (tx) => {
      const session = await tx.workoutSession.findFirst({ where: { id: sessionId, userId: user.id }, include: { exerciseSessions: { include: { setLogs: true } } } });
      ...
      await tx.restPeriod.updateMany({ where: { status: { in: ["RUNNING", "PAUSED"] }, setLog: { exerciseSession: { workoutSession: { userId: user.id } } } }, data: { status: "SKIPPED", skippedAt: completedAt, endsAt: null, pausedRemainingMs: null, pausedRemainingSeconds: null } });
