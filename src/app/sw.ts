...
const runtimeCaching: RuntimeCaching[] = [
  ...
  {
    // Only cache account-neutral offline shells. Authenticated pages can contain
    // another user's server-rendered data and must never enter a shared cache.
    matcher: ({ sameOrigin, request, url }) => sameOrigin && request.mode === "navigate" && isAccountNeutralOfflinePath(url.pathname),
    handler: new NetworkFirst({ cacheName: `${privateCachePrefix}pages-v1`, networkTimeoutSeconds: 4, plugins: [expiration(40, 7 * 24 * 60 * 60)] }),
  },
  {
    matcher: ({ sameOrigin, request, url }) => sameOrigin && request.method === "GET" && isAccountNeutralOfflinePath(url.pathname) && request.headers.get("RSC") === "1",
...
self.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "VICGYM_PREPARE_WORKOUT") event.waitUntil((async () => {
    ...
    await Promise.all(urls.map(async (url) => { try {
      if (!isSafePreparedAsset(url)) return;
      const request = new Request(url, { credentials: "same-origin" }); const response = await fetch(request);
      if (response.ok) await (url.startsWith("/media/") ? mediaCache : pageCache).put(request, response);
    } catch { /* Preparation is best-effort; runtime caching can fill later. */ } }));
  })());
});
