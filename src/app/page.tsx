import { ArrowRight, CalendarDays, CircleDashed, Clock3, Dumbbell, FlaskConical, History, RotateCcw } from "lucide-react";
  ...
  return (
    <AppShell>
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-7 sm:px-8 sm:py-10">
        <div><p className="text-sm font-semibold text-primary">{greetingFor(now, settings?.timezone)}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight sm:text-5xl">Your VicGym home</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">{program ? `Your active ${demo ? "demo " : ""}programme and completed training history are shown below.` : "Paste validated programme JSON in Coach Review to create and activate your programme."}</p></div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
          <section className="rounded-3xl border bg-card p-5 shadow-sm sm:p-7" aria-labelledby="rotation-title">
            <div className="flex items-center justify-between gap-4"><div><p className="text-sm font-semibold text-primary">Upcoming rotation</p><h2 id="rotation-title" className="mt-1 text-xl font-semibold tracking-tight">Workout order</h2></div><CalendarDays className="size-5 text-muted-foreground" aria-hidden="true" /></div>
            {version ? <ol className="mt-5 grid gap-3 sm:grid-cols-2">{version.days.map((day, index) => <li key={day.id} className="flex items-center gap-3 rounded-2xl border bg-background p-4"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-accent text-sm font-semibold text-accent-foreground">{day.rotationOrder}</span><div><p className="font-medium">{day.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{index === 0 ? "First in rotation" : `Rotation ${day.rotationOrder}`}</p></div></li>)}</ol> : <p className="mt-5 text-sm text-muted-foreground">No active programme version is available.</p>}
          </section>
