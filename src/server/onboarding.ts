...
export async function submitFullyPersonalisedOnboarding(prisma: PrismaClient, userId: string, input: FullyPersonalisedInput) {
  return prisma.$transaction(async (tx) => {
    ...
    await tx.onboardingProfile.upsert({ where: { userId }, create: { userId, path: "FULLY_PERSONALISED", goal: input.goal, trainingDaysPerWeek: input.trainingDaysPerWeek, experience: input.experience, sessionLengthMinutes: input.sessionLengthMinutes, cardioPreference: input.cardioPreference, trainingPreferences: input.trainingPreferences, hasLimitations: input.hasLimitations, limitationAreas: [], limitationsText: input.limitationsText || null, limitationReviewRequired: input.hasLimitations, personalPriorities: input.personalPriorities, additionalNotes: input.additionalNotes || null, completedAt: new Date() }, update: { path: "FULLY_PERSONALISED", goal: input.goal, trainingDaysPerWeek: input.trainingDaysPerWeek, experience: input.experience, sessionLengthMinutes: input.sessionLengthMinutes, cardioPreference: input.cardioPreference, trainingPreferences: input.trainingPreferences, hasLimitations: input.hasLimitations, limitationAreas: [], limitationsText: input.limitationsText || null, limitationReviewRequired: input.hasLimitations, personalPriorities: input.personalPriorities, additionalNotes: input.additionalNotes || null, completedAt: new Date() } });
