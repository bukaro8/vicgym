...
import { AUTH_SESSION_TTL_MS, MAGIC_LINK_TTL_MS, MagicLinkRateLimitError, consumeMagicLinkToken, createMagicLinkToken, deleteAuthSessionByToken, findAuthenticatedUser, hashOpaqueToken, magicLinkExpired, sessionCookieOptions } from "@/server/auth";
...
describe("magic-link authentication", () => {
  ...
  it("rejects expired links and consumes a valid link only once while creating a session", async () => {
    ...
    const tx = {
      ...
      user: { upsert: vi.fn(async () => ({ id: "user-1", email: "victor@example.com" })) },
      authSession: { create: authCreate },
    };
    const prisma = { $transaction: (callback: (value: typeof tx) => unknown) => callback(tx) };
    const first = await consumeMagicLinkToken(prisma as never, "secret-link", now);
    expect(first?.user).toEqual({ id: "user-1", email: "victor@example.com" });
  ...
  it("resolves valid server sessions, rejects expired sessions, and deletes logout sessions by hash", async () => {
    const now = new Date("2026-09-08T12:00:00.000Z");
    const findUnique = vi.fn().mockResolvedValue({ expiresAt: new Date(now.getTime() + 60_000), user: { id: "user-1", email: "victor@example.com" } });
    const deleteMany = vi.fn();
    const prisma = { authSession: { findUnique, deleteMany } };
    expect(await findAuthenticatedUser(prisma as never, "session-secret", now)).toEqual({ id: "user-1", email: "victor@example.com" });
