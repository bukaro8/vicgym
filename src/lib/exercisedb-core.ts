...
export async function searchExerciseDb(query: string, options: { after?: string; limit?: number } = {}, fetcher: typeof fetch = fetch, environment?: Record<string, string | undefined>): Promise<ExerciseDbSearchResult> {
  ...
  const params = new URLSearchParams({ search: query.trim(), limit: String(Math.min(Math.max(options.limit ?? 10, 1), 25)) });
  if (options.after) params.set("after", options.after);
  const result = await request(`/exercises/search?${params.toString()}`, fetcher, environment);
...
export async function getPublicExerciseDbExercise(externalExerciseId: string, fetcher: typeof fetch = fetch): Promise<ExerciseDbExercise> {
  if (!externalExerciseId.trim()) throw new Error("Provide an ExerciseDB exercise ID.");
  const response = await fetcher(`https://oss.exercisedb.dev/api/v1/exercises/${encodeURIComponent(externalExerciseId)}`, { headers: { Accept: "application/json" } });
  if (!response.ok) throw new Error(`ExerciseDB public request failed (${response.status}).`);
  const parsed = responseSchema.safeParse(await response.json() as unknown);
  if (!parsed.success || Array.isArray(parsed.data.data)) throw new Error("ExerciseDB public exercise detail returned an invalid response.");
  return parsed.data.data;
}

export function preferredExerciseDbImage(exercise: ExerciseDbExercise): string | null {
  return exercise.imageUrls?.["1080p"] ?? exercise.imageUrls?.["720p"] ?? exercise.imageUrls?.["480p"] ?? exercise.imageUrls?.["360p"] ?? exercise.imageUrl ?? exercise.gifUrl ?? null;
}
