...
import { configureOfflineOwner, getActiveOfflineWorkout, getOfflineOutbox } from "@/lib/offline-db";
...
export function OfflineProvider({ children, userId }: Readonly<{ children: React.ReactNode; userId: string | null }>) {
  configureOfflineOwner(userId);
  ...
  const inspect = useCallback(async () => { if (!userId) { setActive(false); return; } const [workout, outbox] = await Promise.all([getActiveOfflineWorkout(), getOfflineOutbox()]); setActive(Boolean(workout) || outbox.length > 0); if (!navigator.onLine) setState("offline"); else if (outbox.some((mutation) => mutation.lastError)) setState("needs-attention"); else if (outbox.length) setState("saved-local"); else setState("synced"); }, [userId]);
  const retry = useCallback(() => { void syncOfflineMutations().then((next) => { setState(next); void inspect(); }); }, [inspect]);
  useEffect(() => {
    const status = (event: Event) => { const detail = (event as CustomEvent<{ state: SyncState; message?: string }>).detail; setState(detail.state); setMessage(detail.message ?? ""); if (detail.state === "synced") router.refresh(); };
    ...
    const startup = window.setTimeout(() => { if (userId) void inspect().then(() => retry()).catch(() => undefined); }, 0);
