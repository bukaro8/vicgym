...
export function OfflineFinishView({ sessionId }: Readonly<{ sessionId: string }>) {
  ...
  return <AppShell><main className="mx-auto w-full max-w-2xl flex-1 px-4 py-7 sm:px-8 sm:py-10"><a href={`/offline/workout/${sessionId}/${workout.currentExerciseId ?? workout.exercises[0]?.id}`} className="text-sm font-medium text-muted-foreground">← Back to workout</a><p className="mt-7 text-sm font-semibold text-primary">Finish workout · offline</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Review {workout.workoutDayName}</h1><div className="mt-6 grid grid-cols-3 gap-3"><Metric icon={<CheckCircle2 className="size-5 text-primary"/>} value={`${completedExercises}/${workout.exercises.length}`} label="Exercises"/><Metric icon={<ListChecks className="size-5 text-primary"/>} value={`${completedSets}/${plannedSets}`} label="Planned sets"/><Metric icon={<Clock3 className="size-5 text-primary"/>} value={`${minutes}m`} label="Duration"/></div><CardioTimer sessionId={sessionId} initial={{ planned: workout.cardioPlanned ?? false, startedAt: workout.cardioStartedAt ?? null, stoppedAt: workout.cardioStoppedAt ?? null, durationSeconds: workout.cardioDurationSeconds ?? 0 }}/><section className="mt-6 rounded-3xl border bg-card p-5"><h2 className="text-lg font-semibold">Exercise status</h2><ul className="mt-4 space-y-3">{workout.exercises.map((item) => <li key={item.id} className="flex items-center justify-between gap-3 border-t pt-3 first:border-0 first:pt-0"><span className="font-medium">{item.name}{item.isAdHoc && <span className="ml-2 rounded-full bg-accent px-2 py-1 text-xs text-primary">Extra</span>}</span><span className="text-sm text-muted-foreground">{item.sets.filter((set) => set.completedAt).length}/{item.plannedSets} planned sets</span></li>)}</ul></section><div className="mt-6"><FinishWorkout sessionId={sessionId} incomplete={completedSets < plannedSets}/></div></main></AppShell>;
}
