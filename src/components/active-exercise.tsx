...
type SetRow = { id: string; setNumber: number; targetReps: number; actualReps: number | null; weightKg: number | null; loadValue?: number | null; loadTrackingType?: LoadTrackingTypeValue | null; loadEntryMode?: LoadEntryModeValue | null; completedAt: string | null; notes?: string | null };
...
export function ActiveExercise({ sessionId, exerciseSessionId, initialSets, loadTrackingType: suppliedLoadTrackingType, loadEntryMode: suppliedLoadEntryMode, previousId, nextId, offlineSnapshot }: Readonly<{ sessionId: string; exerciseSessionId: string; initialSets: SetRow[]; loadTrackingType?: LoadTrackingTypeValue | null; loadEntryMode?: LoadEntryModeValue | null; previousId: string | null; nextId: string | null; offlineSnapshot?: OfflineWorkout }>) {
  ...
  const [sets, setSets] = useState(initialSets.map((set) => ({ ...set, repsInput: String(set.actualReps ?? set.targetReps), weightInput: displayedLoad(set) === null ? "" : String(displayedLoad(set)), notesInput: set.notes ?? "", dirty: false, saving: false })));
  const [error, setError] = useState("");
  const [adding, setAdding] = useState(false);

  useEffect(() => { void getOfflineWorkout(sessionId).then((workout) => { const local = workout?.exercises.find((exercise) => exercise.id === exerciseSessionId); if (local) setSets(local.sets.map((set) => { const value = (set.loadTrackingType ?? null) === null ? set.weightKg : (set.loadValue ?? null); return { ...set, loadValue: set.loadValue ?? null, loadTrackingType: set.loadTrackingType ?? null, repsInput: String(set.actualReps ?? set.targetReps), weightInput: value === null ? "" : String(value), notesInput: set.notes ?? "", dirty: false, saving: false }; })); }).catch(() => undefined); }, [exerciseSessionId, sessionId]);
  ...
  async function save(id: string) {
    ...
    try {
      ...
      const data = await saveSetLocally({ sessionId, exerciseSessionId, setId: id, actualReps: Number(row.repsInput), loadValue: row.weightInput === "" ? null : Number(row.weightInput), completed: true, notes: row.notesInput });
      setSets((current) => current.map((set) => set.id === id ? { ...set, actualReps: data.set.actualReps, weightKg: data.set.weightKg, loadValue: data.set.loadValue ?? null, loadTrackingType: data.set.loadTrackingType ?? null, completedAt: data.set.completedAt, notes: data.set.notes, notesInput: data.set.notes ?? "", dirty: false, saving: false } : set));
  ...
  async function addSet() {
    ...
    try { await ensureLocalSnapshot(); const set = await addSetLocally(sessionId, exerciseSessionId); setSets((current) => [...current, { ...set, loadValue: set.loadValue ?? null, loadTrackingType: set.loadTrackingType ?? null, repsInput: String(set.actualReps), weightInput: "", notesInput: "", dirty: false, saving: false }]); void syncOfflineMutations(); }
