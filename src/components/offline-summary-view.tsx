...
import { CheckCircle2, Clock3, CloudOff, HeartPulse, ListChecks } from "lucide-react";
...
export function OfflineSummaryView({ sessionId }: Readonly<{ sessionId: string }>) {
  ...
  const completedExercises = data?.workout?.exercises.filter((item) => item.sets.filter((set) => set.completedAt).length >= item.plannedSets).length ?? 0; const duration = data?.workout ? (completedWorkoutDurationMinutes({ ...data.workout, exerciseSessions: data.workout.exercises.map((exercise) => ({ setLogs: exercise.sets })) }) ?? 0) : 0;
  const synchronized = data?.pending === 0;
  return <AppShell><main className="mx-auto w-full max-w-2xl flex-1 px-4 py-10 sm:px-8"><section className="rounded-3xl border bg-card p-6 text-center shadow-sm"><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-accent text-primary"><CheckCircle2 className="size-8"/></span><p className="mt-5 text-sm font-semibold text-primary">Saved on this device</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">{data?.workout?.workoutDayName ?? "Workout complete"}</h1><p className="mt-3 text-muted-foreground">Summary values come from the locally saved session.</p><div className={`mt-6 grid gap-3 text-left ${data?.workout?.cardioPlanned ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}><SummaryMetric icon={<CheckCircle2 className="size-5 text-primary"/>} value={completedExercises} label="Exercises complete"/><SummaryMetric icon={<ListChecks className="size-5 text-primary"/>} value={completedSets} label="Completed sets"/>{data?.workout?.cardioPlanned && <SummaryMetric icon={<HeartPulse className="size-5 text-primary"/>} value={formatCardioDuration(data.workout.cardioDurationSeconds ?? 0)} label="Cardio"/>}<SummaryMetric icon={<Clock3 className="size-5 text-primary"/>} value={`${duration}m`} label="Training time"/></div>{data?.workout && <div className="mt-6 rounded-2xl border bg-background p-4 text-left"><p className="font-semibold">Exercises</p><ul className="mt-3 space-y-2 text-sm">{data.workout.exercises.map((item) => <li key={item.id}>{item.name}{item.isAdHoc && <span className="ml-2 rounded-full bg-accent px-2 py-1 text-xs font-semibold text-primary">Extra</span>}</li>)}</ul></div>}<div className="mt-6 flex items-start gap-3 rounded-2xl border bg-background p-4 text-left">{synchronized ? <CheckCircle2 className="mt-0.5 size-5 shrink-0 text-primary"/> : <CloudOff className="mt-0.5 size-5 shrink-0 text-primary"/>}<p className="text-sm"><strong className="block">{synchronized ? "Synchronized" : "Synchronization pending"}</strong><span className="mt-1 block text-muted-foreground">{!data ? "Checking local changes…" : synchronized ? "All local changes have reached VicGym." : `${data.pending} local change${data.pending === 1 ? "" : "s"} will retry automatically on reconnect, app start, or focus.`}</span></p></div><button type="button" onClick={() => router.push("/")} className="mt-6 inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-primary px-5 font-semibold text-white">Return home</button></section></main></AppShell>;
}
