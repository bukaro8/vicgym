...
export function AdminNavigation({ variant }: { variant: "desktop" | "mobile" }) {
  ...
  if (variant === "desktop") return <nav aria-label="Administrator navigation" className="hidden items-center gap-1 md:flex">{adminItems.map(({ href, label }) => <Link key={href} href={href} aria-current={isActive(pathname, href) ? "page" : undefined} className={cn("rounded-xl px-3 py-2 text-sm font-medium", isActive(pathname, href) ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>{label}</Link>)}<LogoutButton variant="admin-desktop" label="Logout"/></nav>;
  return <nav aria-label="Administrator navigation" className="fixed inset-x-0 bottom-0 z-40 border-t bg-card/95 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden"><div className="mx-auto grid max-w-lg grid-cols-3">{adminItems.map(({ href, mobileLabel, icon: Icon }) => <Link key={href} href={href} aria-current={isActive(pathname, href) ? "page" : undefined} className={cn("flex min-h-16 flex-col items-center justify-center gap-1 text-[0.6875rem] font-medium", isActive(pathname, href) ? "text-primary" : "text-muted-foreground")}><Icon className="size-5"/>{mobileLabel}</Link>)}<LogoutButton variant="admin-mobile" label="Logout"/></div></nav>;
}
