...
async function main() {
  ...
  try {
    assert.deepEqual((await replayOfflineMutations(prisma, batch)).map((item) => item.status), ["applied", "applied", "applied", "applied", "applied"]); assert.deepEqual((await replayOfflineMutations(prisma, batch)).map((item) => item.status), ["duplicate", "duplicate", "duplicate", "duplicate", "duplicate"]);
